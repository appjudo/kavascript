...
# for KavaScript. You define tasks with names and descriptions in a Kakefile,
...
helpers.extend global,
  ...
  # Define a Kake task with a short name, an optional sentence description,
  # and the function to run as the action itself.
  task: (name, description, action) ->
    [action, description] = [description, action] unless action
    tasks[name] = {name, description, action}

  # Define an option that the Kakefile accepts. The parsed options hash,
  # containing all of the command-line options passed, will be made available
  # as the first argument to the action.
  option: (letter, flag, description) ->
    switches.push [letter, flag, description]

  # Invoke another task in the current Kakefile.
  invoke: (name) ->
...
exports.run = ->
  ...
  args = process.argv[2..]
  KavaScript.run fs.readFileSync('Kakefile').toString(), filename: 'Kakefile'
...
# Display the list of Kake tasks in a format similar to `rake -T`
printTasks = ->
...
# When `kake` is invoked, search in the current and all parent directories
# to find the relevant Kakefile.
kakefileDirectory = (dir) ->
  return dir if fs.existsSync path.join dir, 'Kakefile'
