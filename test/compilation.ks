...
test "ensure that carriage returns don't break compilation on Windows", ->
  doesNotThrow -> KavaScript.compile 'one\r\ntwo', bare: on

test "#3089 - don't mutate passed in options to compile", ->
  opts = {}
  KavaScript.compile '1 + 1', opts
  ok !opts.scope 

test "--bare", ->
  eq -1, KavaScript.compile('x = y', bare: on).indexOf 'function'
  ok 'passed' is KavaScript.eval '"passed"', bare: on, filename: 'test'

test "header (#1778)", ->
  header = "// Generated by KavaScript #{KavaScript.VERSION}\n"
  eq 0, KavaScript.compile('x = y', header: on).indexOf header

test "header is disabled by default", ->
  header = "// Generated by KavaScript #{KavaScript.VERSION}\n"
  eq -1, KavaScript.compile('x = y').indexOf header
...
test "reference named hasOwnProperty", ->
  KavaScript.compile 'hasOwnProperty = 0; a = 1'
...
test "#2846: while with empty body", ->
  KavaScript.compile 'while 1 then', {sourceMap: true}

test "#2944: implicit call with a regex argument", ->
  KavaScript.compile 'o[key] /regex/'
