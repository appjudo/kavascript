...
(function() {
  ...
  BANNER = 'Usage: ks [options] path/to/script.ks -- [args]\n\nIf called without options, `ks` will run your script.';

  SWITCHES = [['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive KavaScript REPL'], ['-j', '--join [FILE]', 'concatenate the source KavaScript before compiling'], ['-m', '--map', 'generate source map and save as .js.map files'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['--no-header', 'suppress the "Generated by" header'], ['-o', '--output [DIR]', 'set the output directory for compiled JavaScript'], ['-p', '--print', 'print out the compiled JavaScript'], ['-r', '--require [MODULE*]', 'require the given module before eval or REPL'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-l', '--verbose', 'treat stdio as verbose style kavascript'], ['-t', '--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  findDirectoryIndex = function(source) {
    ...
    ref1 = KavaScript.FILE_EXTENSIONS;
    for (i = 0, len = ref1.length; i < len; i++) {
      ext = ref1[i];
  ...
  compileScript = function(file, input, base) {
    ...
    try {
      ...
      if (o.tokens) {
        return printTokens(KavaScript.tokens(t.input, t.options));
      } else if (o.nodes) {
        return printLine(KavaScript.nodes(t.input, t.options).toString().trim());
      } else if (o.run) {
        KavaScript.register();
        if (opts.prelude) {
          KavaScript["eval"](opts.prelude, t.options);
        }
        return KavaScript.run(t.input, t.options);
      } else if (o.join && t.file !== o.join) {
        if (helpers.isVerbose(file)) {
          t.input = helpers.invertVerbose(t.input);
        }
      ...
      if (KavaScript.listeners('failure').length) {
        return;
      }
  ...
  compileOptions = function(filename, base) {
    ...
    answer = {
      filename: filename,
      verbose: opts.verbose || helpers.isVerbose(filename),
      bare: opts.bare,
  ...
  version = function() {
    return printLine("KavaScript version " + KavaScript.VERSION);
  };
